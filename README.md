...
- From startup JS:
  - modify ```javascript style.display``` to none in order to get ```javascript div```s to disappear
  - input has a ```javascript type=datetime-local``` option in order to enable selecting times
  - ```javascript addEventListener``` can be used to add functionality to html tags
  - using```javascript .className``` can make sure everything is styled correctly
  - when using ```javascript id``` with a ```javascript querySelector```, it will return the value of the first ```javascript input``` with that id, even if it is no longer in use. 
